...
<head>
  ...
  <title>My Storage | Clients  </title>
  <!--     Fonts and icons     -->
...
<?php  include"header.php"; 
   ...
   if(isset($_POST['edit']))
   {
	   $edit=mysqli_query($con,"SELECT * FROM clients WHERE id='".$_POST['id']."' AND user_id='".$_SESSION['user_id']."' ");
   ...
   <form method="post">
	   ...
	   <button type="submit" name="daxil" class="btn btn-dark">Daxil</button>
	   </form>
	   </div>';	
   }
   ...
   if(isset($_POST['beli']))
   {
	   $yox=mysqli_query($con,"SELECT client_id FROM orders WHERE client_id='".$_POST['id']."' ");
	   if(mysqli_num_rows($yox)==0)
	   {
	   $delete=mysqli_query($con,"DELETE FROM clients WHERE id='".$_POST['id']."' ");
   ...
   if(isset($_POST['daxil']))
   {
	   ...
	   if(!empty($ad) && !empty($soyad) && !empty($telefon) && !empty($email) && !empty($shirket))
	   {
		   $unikal=mysqli_query($con,"SELECT * FROM clients WHERE telefon='".$telefon."' AND email='".$email."'");
   
		   $sayisi=mysqli_num_rows($unikal);
		   if($sayisi==0)
		 {
			 $insert=mysqli_query($con,"INSERT INTO clients(ad,soyad,telefon,email,shirket,user_id,tarix)
		   VALUES('".$ad."','".$soyad."','".$telefon."','".$email."','".$shirket."','".$_SESSION['user_id']."','".$tarix."')");
   ...
   if(isset($_POST['he']) && count($_POST['secim'])>0)
   {
	   for($n=0;$n<count($_POST['secim']); $n++)
	   {
		   $sil=mysqli_query($con,"DELETE FROM clients WHERE id='".$_POST['secim'][$n]."'");
	   }
   }
   ...
   {$order = 'ORDER BY ad ASC'; $f1 = '<a href="?f1=DESC"><i class="bi bi-sort-alpha-down-alt"></i></a>';}
   
   elseif(isset($_GET['f1'])=='DESC')
   {$order = 'ORDER BY ad DESC'; $f1 = '<a href="?f1=ASC"><i class="bi bi-sort-alpha-down"></i></a>';}
   ...
   {
	 $axtar = " AND (ad LIKE '%".$_POST['sorgu']."%' OR soyad LIKE '%".$_POST['sorgu']."%' OR telefon LIKE '%".$_POST['sorgu']."%')";
   }
   
   $select=mysqli_query($con,"SELECT * FROM clients WHERE user_id='".$_SESSION['user_id']."' ".$axtar.$order." ");
