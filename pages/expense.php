...
<head>
  ...
  <title>My Storage | Expense </title>
  <!--     Fonts and icons     -->
...
<?php  include"header.php"; 
   ...
   if(isset($_POST['edit']))
   {
     $editing=mysqli_query($con,"SELECT * FROM expense WHERE id='".$_POST['id']."' AND user_id='".$_SESSION['user_id']."'");
   ...
   if(isset($_POST['update']))
   {
   ...
   
       {
           ...
           if($updating==true)
           {echo'<div class="alert alert-success" role="alert">Məlumatlar yeniləndi.</div>';}
           else
           {echo'<div class="alert alert-danger" role="alert">Məlumatlar yenilənmədi.</div>';}
   }
   ...
   if(!isset($_POST['edit']))
   {
       ...
       <button type="submit" name="ok" class="btn btn-dark">Daxil</button>
       </form>
       </div>';	
   }
   ...
   <form method="post">
       <button type="submit" name="d1" class="btn btn-success">Bəli</button>
       <button type="submit" name="d2" class="btn btn-danger">Xeyr</button>
   ...
   if(isset($_POST['d1']))
   {
       $deleting=mysqli_query($con,"DELETE FROM expense WHERE id='".$_POST['id']."'");
   
       if($deleting==true)
       {echo'<div class="alert alert-success" role="alert">Məlumatlar silindi.</div>';}
       else
       {echo'<div class="alert alert-danger" role="alert">Məlumatları silmək mümkün olmadı.</div>';}
   }
   ...
   if(isset($_POST['ok']))
   {
       ...
       if(!empty($teyinat) && !empty($mebleg))
       {
           $inserting=mysqli_query($con,"INSERT INTO expense(teyinat,mebleg,user_id,tarix) VALUES('".$teyinat."','".$mebleg."','".$_SESSION['user_id']."','".$tarix."')");
   ...
   {$order = 'ORDER BY teyinat ASC'; $f1 = '<a href="?f1=DESC"><i class="bi bi-sort-alpha-down-alt"></i></a>';}
   
   elseif(isset($_GET['f1'])=='DESC')
   {$order = 'ORDER BY teyinat DESC'; $f1 = '<a href="?f1=ASC"><i class="bi bi-sort-alpha-down"></i></a>';}
   ...
   {
     $axtar = " AND (teyinat LIKE '%".$_POST['sorgu']."%' OR mebleg LIKE '%".$_POST['sorgu']."%') ";
   }
   
   $secme=mysqli_query($con,"SELECT * FROM expense WHERE user_id='".$_SESSION['user_id']."' ".$axtar.$order." ");
   ...
   while($info=mysqli_fetch_array($secme))
   {
       ...
       <form method="post">
              ...
              <input type="hidden" name="id" value="'.$info['id'].'">
              <button type="submit" name="edit" class="btn btn-success">Redaktə</button>
